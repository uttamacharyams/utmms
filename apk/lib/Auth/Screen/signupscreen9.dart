...
class _PartnerPreferencesPageState extends State<PartnerPreferencesPage> {
  ...
  List<String> _autoGeneratedExpectations = [];
  ...
  void _generateExpectations() {
    final List<String> expectations = [];

    if (_minAge != null && _maxAge != null) {
      expectations.add('Looking for a partner aged between $_minAge and $_maxAge years.');
    }

    if (_minHeight != null && _maxHeight != null) {
      final minCm = _minHeight!.split(' ').first;
      final maxCm = _maxHeight!.split(' ').first;
      expectations.add('Preferred height range: $minCm cm to $maxCm cm.');
    }

    if (_selectedMaritalStatus.isNotEmpty && !_selectedMaritalStatus.contains('Any')) {
      expectations.add('Marital status preference: ${_selectedMaritalStatus.join(", ")}.');
    }

    if (_selectedReligion.isNotEmpty && !_selectedReligion.contains('Any')) {
      expectations.add('Religion: ${_selectedReligion.join(", ")}.');
    }

    if (_selectedCommunity.isNotEmpty && !_selectedCommunity.contains('Any')) {
      expectations.add('Community/caste preference: ${_selectedCommunity.join(", ")}.');
    }

    if (_selectedMotherTongue.isNotEmpty && !_selectedMotherTongue.contains('Any')) {
      expectations.add('Mother tongue preference: ${_selectedMotherTongue.join(", ")}.');
    }

    if (_selectedCountry.isNotEmpty && !_selectedCountry.contains('Any')) {
      expectations.add('Preferred country: ${_selectedCountry.join(", ")}.');
    }

    if (_selectedEducation.isNotEmpty && !_selectedEducation.contains('Any')) {
      expectations.add('Educational qualification: ${_selectedEducation.join(", ")}.');
    }

    if (_selectedOccupation.isNotEmpty && !_selectedOccupation.contains('Any')) {
      expectations.add('Profession preference: ${_selectedOccupation.join(", ")}.');
    }

    if (_selectedDiet.isNotEmpty && !_selectedDiet.contains('Any')) {
    ...
    if (_selectedDrinkAcceptance != null && _selectedDrinkAcceptance != 'Any') {
      expectations.add('Alcohol acceptance: $_selectedDrinkAcceptance.');
    }

    setState(() {
      _autoGeneratedExpectations = expectations;
      if (expectations.isNotEmpty) {
        _otherExpectationsController.text = expectations.join('\n');
      }
    });
  }
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 8),
                        ..._autoGeneratedExpectations.map((item) => Padding(
